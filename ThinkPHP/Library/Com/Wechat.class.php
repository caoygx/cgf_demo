...
class Wechat {
    ...
    public function __construct($token, $appid = '', $key = ''){
        ...
        //TOKEN验证
        if($token){
            //self::auth($token) || exit('验证失败');

            self::$token = $token;
            $this->init();

           /* if(IS_GET){
                if(empty($_GET['echostr'])) echo '没有参数：echostr';
                exit($_GET['echostr']);
            } else {
                self::$token = $token;
                $this->init();
     ...
     * @param  string $picurl      文章缩略图
     */
    public function replySevice(){
        return $this->response(array(func_get_args()), self::MSG_TYPE_SERVICE);
    }
     ...
     * ]
     */
    private static function service($news){
