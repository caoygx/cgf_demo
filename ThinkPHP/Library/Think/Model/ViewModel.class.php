...
class ViewModel extends Model
{
    ...
    public function getDbFields(){
        ...
        foreach ($this->viewFields as $tableName => $tableFields) {
            //var_dump($val);
            foreach ($tableFields as $_field => $v) {
                if (is_numeric($_field)) {
                    if(false !== strpos($v, '*')){
                        $dbFields = M($tableName)->getDbFields();
                        //var_dump($dbFields);exit;
                        $allFields = array_merge($dbFields,$allFields);
                    }

                    $allFields[] = $v;
                } elseif ('_' != substr($_field, 0, 1)) { //别名字段处理
