...
class App
{
    ...
    public static function init()
    {
        ...
        $platform = I('server.HTTP_PLATFORM');
        if($platform == 'android'){
            define('IS_WXAPP', false);
        ...
        if( $_REQUEST['supportWebp'] || strpos($_SERVER['HTTP_ACCEPT'], 'image/webp') !== false || IS_ANDROID ){
            define('IS_WEBP', true);
        }else{
            define('IS_WEBP',false);
        }
    ...
    public static function invokeAction($module, $action)
    {
        ...
        if ($method->isPublic() && !$method->isStatic()) {
			...
			//action开始，获取viewData
			//$r = Hook::listen('getViewData',$class);
			if(!APP_DEBUG){
				$classExist = class_exists('\Common\Behaviors\GetViewDataBehavior');
				if($classExist){
					$behavior = new \Common\Behaviors\GetViewDataBehavior();
					//$method->invokeArgs($module, $args);
					$hasViewData = $behavior->run2($module,$class);
				}
			}
			
			//没有viewData才执行controller
			if(!$hasViewData) {
