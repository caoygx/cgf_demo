...
class Cx extends TagLib
{
    ...
    protected $tags = array(
        ...
        'cssmin'     => array('attr' => 'file,href,type,value,basepath', 'close' => 0),
        'jsmin'     => array('attr' => 'file,href,type,value,basepath', 'close' => 0),
     ...
     * @return string
     */
    public function _min($tag, $content, $isFile = true, $type = '')
    {
        if(APP_DEBUG){
           // return $this->_import($tag, $content, $isFile, $type);

        }

        $module     = isset($tag['module']) ? $tag['module'] : '';
        $file     = isset($tag['file']) ? $tag['file'] : $tag['href'];
        $parseStr = '';
        $endStr   = '';
        // 判断是否存在加载条件 允许使用函数判断(默认为isset)
        ...
        foreach ($array as $val) {
            $val = $this->templateContentReplace($val);
            $val = 'http:'.$val;
            $allContents .= file_get_contents($val);
        }
        ...
        if($type == 'js'){
            $minifier = new \MatthiasMullie\Minify\JS($allContents);
            $allContents = $minifier->minify();

            $jsDir = ASSETS_DIR."/$module/js";
            if(!file_exists($jsDir)) mkdir($jsDir,0777,true);

            $assetsFile = $jsDir."/$assetName.js";
            if(!file_exists($assetsFile) && $validity){
                file_put_contents($assetsFile,$allContents);
            }
            $newUrl = "__PUBLIC__/assets/$module/js/$assetName.js";
            $parseStr = '<script src="' . $newUrl . '" ></script>';
            return $parseStr;
        }else{
            $minifier = new \MatthiasMullie\Minify\CSS($allContents);
            $allContents = $minifier->minify();

            $cssDir = ASSETS_DIR."/$module/css";
            if(!file_exists($cssDir)) mkdir($cssDir,0777,true);
            $assetsFile = $cssDir."/$assetName.css";
            if(!file_exists($assetsFile) && $validity){
                file_put_contents($assetsFile,$allContents);
            }
            $newUrl = "__PUBLIC__/assets/$module/css/$assetName.css";
            $parseStr = '<link rel="stylesheet" type="text/css" href="' . $newUrl . '" />';
            return $parseStr;
        }
    ...
    public function _cssmin($tag, $content)
    {
        return $this->_min($tag, $content, true, 'css');
    }

    public function _jsmin($tag, $content)
    {
        return $this->_min($tag, $content, true, 'js');
    }
