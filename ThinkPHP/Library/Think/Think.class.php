...
class Think
{
    ...
    public static function appException($e)
    {
        ...
        if(!APP_DEBUG){
            ...
            if($ret_format == 'json'){
                error($error['message'],[],$ret_format);
            }elseif ($ret_format == 'jsonp'){ //jsonp
                error($error['message'],[],$ret_format);
            }
        }
        // 发送404信息
    ...
    public static function halt($error)
    {
        ...
        if (APP_DEBUG || IS_CLI) {
            ...
            //$raw = get_raw_request();
            $errorInfo = '';
            $errorInfo .= "[".datetime()." ] ".get_client_ip();
            $errorInfo .= "<br /><br />请求参数 <br /> ".$raw;
            $errorInfo .= "<br />详情 <br />".var_export($error,1);
            //$errorInfo .= "</pre>";
            //echo $errorInfo;exit;
            $errorInfo = str_replace("\n","<br />",$errorInfo);
            $admin_mail = C('admin_eamil');
            if($admin_mail)  sendmail("出错啦！",$errorInfo,$admin_mail,'狗熊');
            ...
            try{
                ...
                if($admin_mail){
                    //$raw = get_raw_request();
                    $raw = "[".datetime()." ] ".get_client_ip()."  ".$_SERVER['REQUEST_URI']."<br />";
                    $errorInfo .= "<br /> 请求id:{$logId} <br />";
                    $errorInfo = $raw.$errorInfo;
                    $errorInfo = str_replace("\n","<br />",$errorInfo);
                    //sendmail("出错啦！",$errorInfo,$admin_mail,'狗熊');
                }
			...
			$ret_format = C("ret_format");
			if($ret_format == 'json'){
				error($error['message'],$error,$ret_format);
			}elseif ($ret_format == 'jsonp'){ //jsonp
				error($error['message'],$error,$ret_format);
			}else{
				// 包含异常页面模板
