...
class CommonModel extends Model
{
     ...
     */
    public function getOneByUid($user_id)
    {
        $r = $this->getList(array("user_id" => $user_id));
     ...
     * @param $id 主键
     */
    public function getOwner($id, $field = "user_id")
    {
     ...
     * @param $field
     */
    public function getOwnInfo($parentID, $field = "user_id")
    {
    ...
    function whereWidthFilterField($where, $parse = null)
    {
        $tableName = $this->getTableName();
        $key = MODULE_NAME.'_'.$tableName.'_'.'list';
        $selectFields = S($key);
        if(empty($selectFields)){
            $tableInfo = new \Common\TableInfo('list');
            $selectFields = $tableInfo->createListSelectFields($tableName);
            S($key,$selectFields);
        }
        return $this->field($selectFields)->where($where, $parse);
    }
